...
| 4 · Design System | `/brand-dna` | **Selettore motore (scegli il migliore per struttura+grafica+CRO):** default `design-system` + `ui-ux-pro-max` → scrive `DESIGN.md`; `firecrawl-website-design-clone` se cloni da sito; **Google Stitch** per spinta grafica/varianti (`stitch-design:generate-design` → `extract-design-md`) o DESIGN.md premium standalone (`stitch-utilities:taste-design`). Stitch (tranne `taste-design`) → Stitch MCP + account, vedi `CONNECTORS.md`. Output sempre normalizzato al template DESIGN.md. |
